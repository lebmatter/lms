...
function updateVideoList() {
  for (var i = 0; i < videos.length; i++) {
    ...
    frappe.call({
      method:
        "lms.lms.doctype.lms_exam_submission.lms_exam_submission.proctor_video_list",
      args: {
        exam_submission: exam_submission,
      },
      success: (data) => {
...
frappe.ready(() => {
  ...
  $("#terminateExam").click(function () {
    ...
    if (result === "Terminate Exam") {
      frappe.call({
        method:
          "lms.lms.doctype.lms_exam_submission.lms_exam_submission.terminate_exam",
        type: "POST",
        args: {
  ...
  function sendProcMessage(message) {
    if (message.trim() !== "") {
      frappe.call({
        method:
          "lms.lms.doctype.lms_exam_submission.lms_exam_submission.post_exam_message",
        type: "POST",
        args: {
          exam_submission: activeChat,
          message: message,
